...
type Decoder interface {
	DecodeArray2D(str []byte) ([][]int, error)
}
...
var tests = []Test{
	...
	{
		Name:  "2d_1_1",
		Input: `[[0]]`, Expect: [][]int{{0}},
	},
	{
		Name:  "2d_2_1",
		Input: `[[0],[1]]`, Expect: [][]int{{0}, {1}},
	},
	{
		Name:  "2d_3_2",
		Input: `[[0,1,2],[3,4],[5]]`, Expect: [][]int{{0, 1, 2}, {3, 4}, {5}},
	},
	func() Test {
		b := Generate2DArray(128, 0, math.MaxInt32, 32, 1234)
		var expect [][]int
	...
	func() Test {
		b := Generate2DArray(1024*1024, 0, math.MaxInt32, 100, 1234)
		var expect [][]int
		if err := json.Unmarshal(b, &expect); err != nil {
			panic(fmt.Errorf("invalid JSON: %v:\n%s", err, string(b)))
		}
...
func (d DecoderJscan) DecodeArray2D(str []byte) ([][]int, error) {
	var s [][]int
	currentIndex := 0
	err := d.Parser.Scan(str, func(i *jscan.Iterator[[]byte]) (err bool) {
		...
		v := i.Value()
		// vi, errp := strconv.Atoi(string(v))
		vi, errp := strconv.Atoi(unsafe.String(unsafe.SliceData(v), len(v)))
		if errp != nil {
...
func (DecoderEncodingJson) DecodeArray2D(str []byte) ([][]int, error) {
	var s [][]int
...
func (d DecoderJsoniterUnmarshal) DecodeArray2D(str []byte) ([][]int, error) {
	var s [][]int
...
func (d DecoderJsoniterIterator) DecodeArray2D(str []byte) (a [][]int, err error) {
	...
	d.Iterator.ReadArrayCB(func(i *jsoniter.Iterator) bool {
		...
		currentIndex = len(a)
		a = append(a, make([]int, 0, 32))
		return i.ReadArrayCB(func(i *jsoniter.Iterator) bool {
			if i.WhatIsNext() != jsoniter.NumberValue {
				err = ErrUnexpectedValue
				return false
			}
			a[currentIndex] = append(a[currentIndex], i.ReadInt())
			return true
		})
	})
	if err != nil {
...
func (d DecoderGofasterJx) DecodeArray2D(str []byte) (a [][]int, err error) {
...
func (d DecoderValyalaFastjson) DecodeArray2D(str []byte) (a [][]int, err error) {
...
func Generate2DArray(
	maxMemoryBytes, intMin, intMax, maxArrayLen int, randSeed int64,
) []byte {
	...
	for len(b) < maxMemoryBytes {
		...
		for i := 0; i < subArrayLen; i++ {
			...
			numBuf = strconv.AppendInt(numBuf, int64(randInt(intMin, intMax)), 10)
